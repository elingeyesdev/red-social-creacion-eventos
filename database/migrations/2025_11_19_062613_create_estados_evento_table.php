...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('estados_evento', function (Blueprint $table) {
            $table->id();
            $table->string('codigo', 50)->unique()->comment('Código único del estado (ej: borrador, publicado)');
            $table->string('nombre', 100)->comment('Nombre del estado');
            $table->text('descripcion')->nullable()->comment('Descripción del estado');
            $table->enum('tipo', ['evento', 'mega_evento', 'ambos'])->default('ambos')->comment('Tipo de evento al que aplica');
